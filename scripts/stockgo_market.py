...
AI_STRATEGIES = (
    ("database", "ai_db_challenger", 3, "AI 資料庫組"),
    ("web", "ai_web_challenger", 3, "AI 純網路組"),
)
...
def _personal_portfolio(path: Path | None) -> dict[str, Any]:
    """Load and reconcile the owner-provided brokerage snapshot."""
    empty = {
        "status": "not_configured",
        "currency": "TWD",
        "baseline_as_of": None,
        "baseline_market_value_twd": None,
        "baseline_unrealized_pnl_twd": None,
        "cost_basis_twd": None,
    ...
    if payload.get("currency") != "TWD":
        errors.append("currency_must_be_twd")
    try:
        baseline_as_of = dt.datetime.fromisoformat(str(payload.get("as_of") or ""))
        if baseline_as_of.tzinfo is None:
            raise ValueError
        baseline_as_of_text = baseline_as_of.astimezone(TAIPEI).isoformat(timespec="seconds")
    except ValueError:
        baseline_as_of_text = None
    ...
    for raw in payload.get("positions") or []:
        ...
        reference_price = raw.get("broker_reference_price")
        market_value = raw.get("broker_market_value_twd")
        unrealized_pnl = raw.get("broker_unrealized_pnl_twd")
        if not code.isdigit() or not 4 <= len(code) <= 6 or code in seen_codes:
            errors.append(f"invalid_or_duplicate_code:{code}")
            continue
        if isinstance(shares, bool) or not isinstance(shares, int) or shares <= 0:
            errors.append(f"invalid_shares:{code}")
            continue
        numeric_values = (reference_price, market_value, unrealized_pnl)
        if any(isinstance(value, bool) or not isinstance(value, (int, float))
               or not math.isfinite(float(value)) for value in numeric_values):
            errors.append(f"invalid_numbers:{code}")
            continue
        reference_price = float(reference_price)
        market_value = int(market_value)
        unrealized_pnl = int(unrealized_pnl)
        if reference_price <= 0 or market_value <= 0:
            errors.append(f"non_positive_value:{code}")
            continue
        if abs(reference_price * shares - market_value) > 1.0:
            errors.append(f"reference_value_mismatch:{code}")
            continue
        cost_basis = market_value - unrealized_pnl
        if cost_basis <= 0:
            errors.append(f"invalid_cost_basis:{code}")
            continue
        seen_codes.add(code)
        positions.append({
            "code": code,
            "name": name,
            "shares": shares,
            "cost_basis_twd": cost_basis,
            "average_cost": cost_basis / shares,
            "broker_reference_price": reference_price,
            "broker_market_value_twd": market_value,
            "broker_unrealized_pnl_twd": unrealized_pnl,
        })

    summary = payload.get("summary") if isinstance(payload.get("summary"), dict) else {}
    baseline_market_value = sum(row["broker_market_value_twd"] for row in positions)
    baseline_unrealized_pnl = sum(row["broker_unrealized_pnl_twd"] for row in positions)
    if summary.get("market_value_twd") != baseline_market_value:
        errors.append("summary_market_value_mismatch")
    if summary.get("unrealized_pnl_twd") != baseline_unrealized_pnl:
        errors.append("summary_unrealized_pnl_mismatch")
    if not positions:
        errors.append("no_valid_positions")
    if errors:
        return {**empty, "status": "invalid", "validation_errors": errors}

    cost_basis = baseline_market_value - baseline_unrealized_pnl
    return {
        "status": "ready",
        "currency": "TWD",
        "source": "broker_snapshot",
        "baseline_as_of": baseline_as_of_text,
        "baseline_market_value_twd": baseline_market_value,
        "baseline_unrealized_pnl_twd": baseline_unrealized_pnl,
        "cost_basis_twd": cost_basis,
        "baseline_return": baseline_unrealized_pnl / cost_basis,
        "positions": positions,
        "validation_errors": [],
    }
...
def _comparison_horizons(
    ...
    shadow_strategy_id: str, shadow_strategy_version: int,
) -> list[dict[str, Any]]:
    """Return one reconciled, public metric shape for 5/20/60-day views."""
    horizons = []
    for horizon_days, source_id, source_version in (
        (5, strategy_id, strategy_version),
        (20, strategy_id, strategy_version),
        (60, shadow_strategy_id, shadow_strategy_version),
    ):
        row = conn.execute(
            "select count(*) total_rows,"
            "sum(case when settlement_status='settled' then 1 else 0 end) settled,"
            "sum(case when settlement_status='pending' then 1 else 0 end) pending,"
            "sum(case when settlement_status='unavailable' then 1 else 0 end) unavailable,"
            "avg(case when settlement_status='settled' then direction_hit end) hit_rate,"
            "avg(case when settlement_status='settled' and alpha_ret>0 then 1.0 "
            "         when settlement_status='settled' then 0.0 end) beat_rate,"
            "avg(case when settlement_status='settled' then net_ret end) avg_return,"
            "avg(case when settlement_status='settled' then benchmark_ret end) avg_benchmark,"
            "avg(case when settlement_status='settled' then alpha_ret end) avg_alpha "
            "from strategy_paper_log where strategy_id=? and strategy_version=? "
            "and horizon_days=?",
            (source_id, source_version, horizon_days),
        ).fetchone()
        ...
        status = (
            "not_started" if total == 0 else
            "collecting" if settled == 0 else
            "results_available"
        )
        horizons.append({
            "horizon_days": horizon_days,
            "source_strategy_id": source_id,
            "shadow_only": horizon_days == 60,
            "status": status,
            "total_rows": total,
            ...
            "average_benchmark": row["avg_benchmark"],
            "average_alpha": row["avg_alpha"],
        })
    return horizons


def _attach_comparison_outcomes(
    conn: sqlite3.Connection, rows: list[dict[str, Any]], *,
    strategy_id: str, strategy_version: int,
    shadow_strategy_id: str, shadow_strategy_version: int,
) -> None:
    """Attach per-code horizon states without duplicating paper rows publicly."""
    outcome_rows = conn.execute(
        "select signal_date,code,horizon_days,settlement_status,exit_date,"
        "net_ret,benchmark_ret,alpha_ret "
        "from strategy_paper_log where "
        "((strategy_id=? and strategy_version=? and horizon_days in (5,20)) "
        "or (strategy_id=? and strategy_version=? and horizon_days=60))",
        (strategy_id, strategy_version, shadow_strategy_id, shadow_strategy_version),
    ).fetchall()
    outcomes = {
        (str(row["signal_date"]), str(row["code"]), int(row["horizon_days"])): {
            "settlement_status": row["settlement_status"],
            "exit_date": row["exit_date"],
            "net_return": row["net_ret"],
            "benchmark_return": row["benchmark_ret"],
    ...
    benchmark_dates = [
        ...
        for item in conn.execute(
            "select date from prices where code=? order by date", ("0050",)
        ).fetchall()
    ] if _table_exists(conn, "prices") else []
    for row in rows:
        entry_date = str(row.get("entry_date") or "")
        sessions_elapsed = sum(
            1 for date in benchmark_dates if entry_date and date >= entry_date
        )
        row["horizons"] = {
            str(horizon): outcomes.get((str(row["signal_date"]), str(row["code"]), horizon), {
                "settlement_status": "not_started" if horizon == 60 else "unavailable",
                "exit_date": None,
                "net_return": None,
        ...
        for horizon, outcome in row["horizons"].items():
            outcome["sessions_elapsed"] = (
                min(sessions_elapsed, int(horizon))
                if outcome["settlement_status"] in {"pending", "settled"}
                and entry_date else 0
            )
...
def _paper_experiment(paper_db: Path | None, paper_reports: Path | None) -> dict[str, Any]:
    ...
    with closing(_connect(paper_db)) as conn:
        ...
        _attach_comparison_outcomes(
            conn, primary,
            strategy_id=PULLBACK_STRATEGY_ID,
            strategy_version=PULLBACK_STRATEGY_VERSION,
            shadow_strategy_id="core_revenue_60d_shadow",
            shadow_strategy_version=1,
        )
        core_horizons = _comparison_horizons(
            conn,
            strategy_id=PULLBACK_STRATEGY_ID,
            strategy_version=PULLBACK_STRATEGY_VERSION,
            shadow_strategy_id="core_revenue_60d_shadow",
        ...
        for arm, strategy_id, version, label in AI_STRATEGIES:
            ...
            if _table_exists(conn, "ai_challenger_run"):
                for run_row in conn.execute(
                        ...
                        "where strategy_id=? and strategy_version=? "
                        "group by status", (strategy_id, version)):
                    run_counts[str(run_row["status"])] = int(run_row["n"])
                run_dates = {
                    str(row["signal_date"])
                    for row in conn.execute(
                        "select signal_date from ai_challenger_run "
                        "where strategy_id=? and strategy_version=?",
                        (strategy_id, version))
                }
                latest_row = conn.execute(
                    "select signal_date,run_at,status,model_id,no_run_reason "
                    "from ai_challenger_run where strategy_id=? "
                    "and strategy_version=? order by run_at desc limit 1",
                    (strategy_id, version),
                ).fetchone()
            ...
            benchmark_dates = []
            if registered_date and has_prices:
                benchmark_dates = [
            ...
            shadow_strategy_id = (
                "ai_db_60d_shadow" if arm == "database" else "ai_web_60d_shadow"
            )
            _attach_comparison_outcomes(
                conn, public_rows,
    ...
    comparison_groups = [{
        ...
        "latest_batch_count": len(latest_signals),
    ...
    comparison_groups.extend({
        "key": group["arm"],
        "label": "AI 資料庫" if group["arm"] == "database" else "純網路 AI",
        "strategy_id": group["strategy_id"],
        "strategy_version": group["strategy_version"],
        "latest_signal_date": group["latest_signal_date"],
        "latest_batch_count": len(group["latest_picks"]),
        "current_tracking_count": sum(
            1 for row in group["positions"]
            if row["settlement_status"] == "pending" and row["entry_date"]
        ),
        "awaiting_entry_count": len(group["awaiting_latest_entry"]),
        "positions": group["positions"],
        "horizons": group["horizons"],
        "successful_signal_days": group["successful_signal_days"],
        "no_run_days": group["no_run_days"],
        "missing_run_signal_dates": group["missing_run_signal_dates"],
        "run_status": (group["latest_run"] or {}).get("status") or "waiting",
...
def build_context(
    ...
    *,
    ...
    personal_holdings_path: Path | None = None,
) -> dict[str, Any]:
    ...
    personal_portfolio = _personal_portfolio(personal_holdings_path)
    ...
    holding_codes = {
        str(row.get("code") or "")
        for row in personal_portfolio.get("positions") or []
        if row.get("code")
    }
    public_codes = sorted({
        str(row.get("code") or "")
    ...
    with closing(_connect(formal_db)) as conn:
        ...
        if holding_codes and _table_exists(conn, "prices"):
            placeholders = ",".join("?" for _ in holding_codes)
            query = f"""
                select p.code,p.date,p.close
                from prices p
                join (
                    select code,max(date) date from prices
                    where code in ({placeholders}) group by code
                ) latest on latest.code=p.code and latest.date=p.date
            """
            for row in conn.execute(query, sorted(holding_codes)):
                latest_price_by_code[str(row["code"])] = dict(row)
    ...
    for row in personal_portfolio.get("positions") or []:
        ...
        row["market"] = market
        row["quote_channel"] = f"{channel_market}_{code}.tw" if channel_market else None
        row["latest_price"] = latest_price.get("close")
        row["latest_price_date"] = latest_price.get("date")
    ...
    quote_channels = [
        row["quote_channel"]
        for row in [
            *candidates, *avoids, *ai_public_rows,
            *(personal_portfolio.get("positions") or []),
        ]
    ...
    return {
        ...
        "sources": [
            *([{
                "name": "本人券商持股截圖",
                "freshness": personal_portfolio.get("baseline_as_of"),
                "note": "股數與成本基準；成本由截圖市值減未實現損益反推，不含帳號資料。",
            }] if personal_portfolio.get("status") == "ready" else []),
            {
                "name": "StockGO 正式庫",
...
def _round_twd(value: float) -> int:
    return int(math.floor(value + 0.5))


def _mark_personal_portfolio(
    source: dict[str, Any], quote_by_code: dict[str, dict[str, Any]]
) -> dict[str, Any]:
    """Mark actual holdings while preserving the brokerage cost baseline."""
    portfolio = copy.deepcopy(source)
    if portfolio.get("status") != "ready":
        return portfolio
    baseline_date = str(portfolio.get("baseline_as_of") or "")[:10]
    marked_positions: list[dict[str, Any]] = []
    quote_times: list[str] = []
    daily_coverage = 0
    for source_row in portfolio.get("positions") or []:
        row = dict(source_row)
        code = str(row.get("code") or "")
        quote = quote_by_code.get(code) or {}
        candidates: list[tuple[str, int, float, str, str | None, float | None]] = []
        if quote.get("value") is not None:
            quote_at = str(quote.get("quote_at") or "")
            candidates.append((
                quote_at[:10], 3, float(quote["value"]),
                str(quote.get("price_basis") or "last_trade"),
                quote_at or None, quote.get("reference"),
            ))
        if row.get("latest_price") is not None and row.get("latest_price_date"):
            latest_date = str(row["latest_price_date"])
            candidates.append((
                latest_date, 2, float(row["latest_price"]), "latest_close",
                latest_date, None,
            ))
        candidates.append((
            baseline_date, 1, float(row["broker_reference_price"]),
            "broker_snapshot", portfolio.get("baseline_as_of"), None,
        ))
        _, _, current_price, price_basis, price_as_of, day_reference = max(
            candidates, key=lambda item: (item[0], item[1])
        )
        shares = int(row["shares"])
        cost_basis = int(row["cost_basis_twd"])
        market_value = _round_twd(current_price * shares)
        unrealized_pnl = market_value - cost_basis
        row.update({
            "current_price": current_price,
            "current_price_at": price_as_of,
            "current_price_basis": price_basis,
            "market_value_twd": market_value,
            "unrealized_pnl_twd": unrealized_pnl,
            "total_return": unrealized_pnl / cost_basis,
            "allocation": None,
            "day_change_pct": None,
            "day_pnl_twd": None,
        })
        if price_basis not in {"latest_close", "broker_snapshot"} and day_reference:
            day_reference = float(day_reference)
            if day_reference > 0:
                row["day_change_pct"] = current_price / day_reference - 1.0
                row["day_pnl_twd"] = market_value - _round_twd(day_reference * shares)
                daily_coverage += 1
        if price_as_of:
            quote_times.append(str(price_as_of))
        marked_positions.append(row)

    total_market_value = sum(row["market_value_twd"] for row in marked_positions)
    total_cost_basis = sum(row["cost_basis_twd"] for row in marked_positions)
    total_unrealized_pnl = total_market_value - total_cost_basis
    for row in marked_positions:
        row["allocation"] = (
            row["market_value_twd"] / total_market_value if total_market_value else None
        )
    portfolio.update({
        "positions": marked_positions,
        "market_value_twd": total_market_value,
        "unrealized_pnl_twd": total_unrealized_pnl,
        "total_return": total_unrealized_pnl / total_cost_basis if total_cost_basis else None,
        "day_pnl_twd": (
            sum(int(row["day_pnl_twd"]) for row in marked_positions)
            if daily_coverage == len(marked_positions) else None
        ),
        "day_quote_coverage": daily_coverage,
        "priced_positions": len(marked_positions),
        "latest_price_at": max(quote_times) if quote_times else None,
    })
    return portfolio
...
def build_snapshot(
    ...
    *,
    ...
    personal_portfolio = _mark_personal_portfolio(
        context.get("personal_portfolio") or {}, quote_by_code
    )
    ...
    for group in experiment.get("ai_groups") or []:
        for key in ("latest_picks", "tracking_picks", "awaiting_latest_entry"):
            for row in group.get(key) or []:
                _apply_entry_performance(row, quote_by_code)
    for group in experiment.get("comparison_groups") or []:
...
def export_market(
    *,
    ...
    context = build_context(
        ...
        personal_holdings_path=personal_holdings_path,
    )
    output_dir.mkdir(parents=True, exist_ok=True)
